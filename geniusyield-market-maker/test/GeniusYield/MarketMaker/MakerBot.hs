...
evolveStrategy
  ...
  -> MVar [LogData]
  -> StateT MBFret IO ()
evolveStrategy runStrategy mb@MakerBot {mbUser, mbDelay, mbToken} netId providers pp di logRef = do
  ...
  mbTest $ \ppStatus -> do
    ...
    case mbFret of
      MBReady -> do
        (newActions, controller) <- lift $ runStrategy ppStatus pp mbUser mbToken
        case controller of
          UACNormal     -> do
        ...
        if mbs1Relax > pccAfterExitRelaxAim1 cfg then do
             ...
             put MBReady
                   ...
                   then do
                     ...
                     pe <- lift $ priceEstimate' ppStatus pp mbToken
