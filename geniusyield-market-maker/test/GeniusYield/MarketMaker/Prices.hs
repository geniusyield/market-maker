...
buildGetQuota :: PPStatus -> PriceCommonCfg -> PricesProviderCfg -> GetQuota

buildGetQuota PPStatus {..} PriceCommonCfg {..} MaestroConfig {..} = GetQuota $ \mmtp -> do
  ...
  let info = head ohlInfo
      ...
      adjustedPrice = price * precisionDiff + maestroOffset
...
buildGetQuota PPStatus {..} PriceCommonCfg {..} TaptoolsConfig {..} = GetQuota $ \mmtp -> case pccNetworkId of
  GYMainnet -> do
    commodity :: MMToken <-
...
buildGetQuotas :: PPStatus -> PricesProviders -> [GetQuota]
buildGetQuotas ppStatus PP {pricesAggregatorPP = PPA {..}} = buildGetQuota ppStatus ppaCommonCfg <$> ppaPricesCluster
...
priceEstimate :: PPStatus -> PricesProviders -> MMTokenPair -> IO PriceIndicator
priceEstimate ppStatus pp mmtp = do
  let pproviders = buildGetQuotas ppStatus pp
...
priceEstimate' :: PPStatus -> PricesProviders -> MMToken -> IO PriceIndicator
priceEstimate' ppStatus pp mmToken = priceEstimate ppStatus pp mmtp
  where
